...
class Spike(pygame.sprite.Sprite):
    def __init__(self,screen, x, y, type):
        ...
        if type == "R":
            self.IMGS = self.ss.images_at(SPIKE_R, colorkey = WHITE)
        if type == "D":
            self.IMGS = self.ss.images_at(SPIKE_D, colorkey = WHITE)
        if type == "L":
            self.IMGS = self.ss.images_at(SPIKE_L, colorkey = WHITE)

        self.image = self.IMGS[random.randint(0,1)] 
